					...
					<div class="col-lg-10">
						<select class="form-control" id="select">
							<c:forEach var="depart" items="${departmentList}">
								<option
									<c:if test="${userInfo.udepartmentName == depart.CODE_NAME}">selected="selected"</c:if>>${depart.CODE_NAME}</option>
							</c:forEach>
						</select>
					</div>
				</div>
					...
					<div class="col-lg-10">
						<c:choose>
							<c:when test="${userInfo.enabled == 1}">
								<a href="#" id="admit" class="btn btn-success btn-xs"
									alt="${userInfo.uno}">승인</a>
							</c:when>
							<c:otherwise>
								<a href="#" id="deny" class="btn btn-warning btn-xs"
